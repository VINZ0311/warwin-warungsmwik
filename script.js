    ...
    function setupEventListeners() {
      ...
      document.addEventListener('click', (e) => {
        ...
        // Admin chat user selection - PERBAIKAN DISINI
        if (e.target.closest('.chat-user')) {
          ...
          renderAdminChatMessages(username); // Tambahkan ini
        }
      });
    }
    ...
    function renderAdminChats() {
      ...
      // Jika ada activeChatUser, render pesannya
      if (activeChatUser) {
        renderAdminChatMessages(activeChatUser);
      } else {
        // Jika tidak ada activeChatUser, tampilkan pesan default
        adminChatMessages.innerHTML = `
          <div class="no-chat-selected">
            <i class="fas fa-comment-alt"></i>
            <p>Pilih pelanggan untuk memulai chat</p>
    ...
    function renderAdminChatMessages(username) {
      ...
      chats[username].forEach(msg => {
        const msgElement = document.createElement('div');
        msgElement.className = `chat-message ${msg.sender}`;
    ...
    // Login success - PERBAIKAN DISINI (tambahkan pengecekan currentUser)
    function loginSuccess() {
      ...
      if (currentUser) {
        ...
        // Hanya tampilkan icon CS jika bukan admin (karena admin punya panel khusus)
        if (!currentUser.isAdmin) {
